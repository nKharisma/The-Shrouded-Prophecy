...
public class QuestLogScrollingList : MonoBehaviour
{
    ...
    // for it to work, you'll need to change the QuestInfoSO
    // private void Start()
    // {
    //     for(int i = 0; i < 20; i++)
    //     {
    //         QuestInfoSO questInfoTest = ScriptableObject.CreateInstance<QuestInfoSO>();
    //         questInfoTest.questID = "test_" + i;
    ...
    public QuestLogButton CreateButtonIfNotExists(Quest quest, UnityAction selectAction)
    {
        ...
        // only create the button if we haven't seen thsi quest id before
        if(!idToButtonMap.ContainsKey(quest.questInfoSO.questID))
        {
            questLogButton = InstantiateQuestLogButton(quest, selectAction);
        }
        else
        {
            questLogButton = idToButtonMap[quest.questInfoSO.questID];
    ...
    private QuestLogButton InstantiateQuestLogButton(Quest quest, UnityAction selectAction)
    {
        ...
        questLogButton.gameObject.name = quest.questInfoSO.questID + "_button";
        ...
        questLogButton.Initialize(quest.questInfoSO.displayName, () => {
            selectAction();
            UpdateScrolling(buttonRectTransform);
        });

        // questLogButton.Initialize(quest.questInfoSO.displayName, selectAction);
        // add to map to keep track of the new button
        idToButtonMap[quest.questInfoSO.questID] = questLogButton;
        return questLogButton;
    }
